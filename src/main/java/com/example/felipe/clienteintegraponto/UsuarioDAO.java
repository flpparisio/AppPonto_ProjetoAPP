...
public class UsuarioDAO {
    ...
    private static final String URL = "http://192.168.0.108:8080/WebServicePonto/services/UsuarioDAO?wsdl";
    private static final String NAMESPACE = "http://ponto.sistema.integra.com.br";

    private static final String BUSCAR_USUARIO = "buscarUsuario";

    public TelaModuloCadastroClasse busarUsuario(long numCpf) {
        TelaModuloCadastroClasse usuario = null;

        SoapObject buscarUsuario = new SoapObject(NAMESPACE, BUSCAR_USUARIO);
        buscarUsuario.addProperty("numCpf", "62150880059");
        ...
        try {
            http.call("urn:" + BUSCAR_USUARIO, envelope);
            SoapObject resposta = (SoapObject) envelope.getResponse();

            usuario = new TelaModuloCadastroClasse();
            usuario.setNomFun(resposta.getProperty("nomFun").toString());
            usuario.setDesCar(resposta.getProperty("desCar").toString());

            String fotEmp = resposta.getProperty("fotEmp").toString();
            byte[] bitmap = Base64.decode(fotEmp, Base64.DEFAULT);
            usuario.setFotEmp(bitmap);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
